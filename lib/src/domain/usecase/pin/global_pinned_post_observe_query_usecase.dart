...
class GlobalPinnedPostObserveQueryUseCase extends ObserverUseCase<
    List<AmityPinnedPost>, QueryGlobalPinnedPostRequest> {
  final PinRepo pinRepo;
  final PostRepo postRepo;
  final PagingIdRepo pagingIdRepo;
  final PinnedPostComposerUsecase pinnedPostComposerUsecase;
  final nonce = AmityNonce.GLOBAL_PINNED_POST_LIST.value;
  ...
  StreamController<List<AmityPinnedPost>> listen(
    ...
    pinRepo.listenGlobalPinnedPosts(request).listen((event) async {
      _onChanges(streamController, request);
    });
  ...
  void _onChanges(
    ...
    final pinnedPostEntities = pinRepo.getGlobalPinnedPostEntities(request);
    ...
    if (pinnedPostEntities.isEmpty || pagingIds.isEmpty) {
      ...
      final pinnedPosts = pinnedPostEntities
        ...
        ..sort((a, b) {
          var positionA =
              pagingIds.firstWhere((p) => p.id == a.pinId).position ?? 0;
          var positionB =
              pagingIds.firstWhere((p) => p.id == b.pinId).position ?? 0;
          return positionA.compareTo(positionB);
        });

      Stream.fromIterable(pinnedPosts)
          .asyncMap((element) => pinnedPostComposerUsecase.get(element))
          .toList()
          .then((value) {
        final excludeDeletedPosts =
            value.where((element) => element.post?.isDeleted == false).toList();
        if (!streamController.isClosed) {
